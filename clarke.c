...
int get_clarke(int a_phase, int b_phase, int c_phase, MC_ALPHABETA_T *mcAlphaBeta)
{
    ...
    if (a_phase > MAX_i_phase || b_phase > MAX_i_phase || c_phase > MAX_i_phase||a_phase < MIN_i_phase || b_phase < MIN_i_phase || c_phase < MIN_i_phase)
    {
        error_fl = 1;
    ...
    mcAlphaBeta->alpha = a_phase;
    mcAlphaBeta->beta = -((a_phase * (1 / (sqrt(3)))) + (2 * b_phase * (1 / (sqrt(3)))));
